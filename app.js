...
app.post("/login/", async (request, response) => {
  ...
  if (dbUserExist !== undefined) {
    ...
    if (checkPassword === true) {
      ...
      const jwtToken = jwt.sign(payload, "MANIDEEP");
...
const authenticationToken = (request, response, next) => {
  ...
  if (jwtToken !== undefined) {
    jwt.verify(jwtToken, "MANIDEEP", async (error, payload) => {
      if (error) {
        response.status(401);
        response.send("Invalid JWT Token");
      } else {
...
app.get("/user/tweets/", authenticationToken, async (request, response) => {
  ...
  const getTweets = `select * from tweet where tweet_id in ${getTweetIdss}`;
  response.send(apiOutput(getTweets));
});
